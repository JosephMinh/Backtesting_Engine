...
SMOKE_SOURCE = REPO_ROOT / "rust" / "opsd" / "src" / "bin" / "readiness_smoke.rs"
...
def run_smoke_scenarios(binary_path: Path, artifact_root: Path) -> str:
    ...
    env = os.environ.copy()
    env["READINESS_SMOKE_ARTIFACT_ROOT"] = str(artifact_root)
    env["TMPDIR"] = str(SAFE_TMPDIR)
    env["TMP"] = str(SAFE_TMPDIR)
    env["TEMP"] = str(SAFE_TMPDIR)
    result = subprocess.run(
        [
            str(binary_path),
            "--exact",
            "smoke_tests::scenario_sweep_emits_readiness_states",
            "--nocapture",
        ],
        cwd=REPO_ROOT,
        env=env,
...
def main() -> int:
    ...
    if rustc is None:
        raise RuntimeError("rustc is required for opsd readiness smoke")

    TMPDIR.mkdir(parents=True, exist_ok=True)
    artifact_root = TMPDIR / (
        "backtesting_engine_opsd_readiness_"
        + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%S")
    )
    binary_path = TMPDIR / f"readiness_smoke_tests_{secrets.token_hex(4)}"
