...
RECORD_DEFINITIONS: tuple[RecordDefinition, ...] = (
    ...
    RecordDefinition(
        record_id="candidate_record",
        title="Candidate record",
        storage_class=StorageClass.CANONICAL_METADATA,
        plan_section="3.3",
        description="Candidate identity and approved executable bindings.",
        queryable_when_telemetry_pruned=True,
        durable_when_telemetry_pruned=True,
        retention_independent=True,
        fields=(
            _field("candidate_id", FieldClass.STATE_OF_TRUTH, "Stable candidate identifier."),
            _field(
                "release_manifest_id",
                FieldClass.CANONICAL_REFERENCE,
                "Frozen release manifest backing the candidate.",
                LifecycleNeed.REPLAY,
                LifecycleNeed.PROMOTION,
            ),
            _field(
                "research_run_id",
                FieldClass.CANONICAL_REFERENCE,
                "Research run that emitted the candidate.",
                LifecycleNeed.REPLAY,
            ),
            _field(
                "execution_contract_profile_id",
                FieldClass.STATE_OF_TRUTH,
                "Execution contract/profile binding approved for evaluation.",
                LifecycleNeed.REPLAY,
                LifecycleNeed.PROMOTION,
            ),
            _field(
                "candidate_class",
                FieldClass.STATE_OF_TRUTH,
                "Strictness and promotion lane of the candidate.",
                LifecycleNeed.PROMOTION,
            ),
        ),
    ),
    RecordDefinition(
        record_id="readiness_record",
        title="Readiness record",
        storage_class=StorageClass.CANONICAL_METADATA,
        plan_section="3.3",
        description="Formal readiness review outcome before promotion or session use.",
        queryable_when_telemetry_pruned=True,
        durable_when_telemetry_pruned=True,
        retention_independent=True,
        fields=(
            _field("readiness_record_id", FieldClass.STATE_OF_TRUTH, "Stable readiness record identifier."),
            _field(
                "candidate_id",
                FieldClass.CANONICAL_REFERENCE,
            ...
            _field(
                "broker_conformance_digest",
                FieldClass.STATE_OF_TRUTH,
                "Digest of broker and contract conformance checks.",
                LifecycleNeed.PROMOTION,
            ),
            _field(
                "reconciliation_status",
                FieldClass.STATE_OF_TRUTH,
                "Whether required reconciliation evidence is green.",
                LifecycleNeed.PROMOTION,
            ),
            _field(
                "operator_signoff_id",
